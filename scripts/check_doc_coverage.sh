...
if [[ "${1:-}" == "--write" ]]; then
    {
        ...
        echo "#   clojure.pprint — ~50 of these are cljw-internal \`cl-*\` formatter"
        echo "#     helpers that upstream keeps private. They are a leaked API surface"
        echo "#     rather than a documentation gap; the fix is \`defn-\`, not prose."
        echo "#   clojure.math   — upstream's docstrings all end in a java.lang.Math"
        echo "#     Javadoc URL, so importing them verbatim would ship 45 Java-8 links"
        echo "#     as the reference for a runtime with no java.lang.Math. These need"
        echo "#     authored prose."
