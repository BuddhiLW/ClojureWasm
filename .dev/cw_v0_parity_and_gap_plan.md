...
| G11 | `--list-vars` / tier introspection                                   | **Yes**                        | `cljw --list-vars` over `compat_tiers.yaml` + `placement.yaml`; shared completion source for G1 nREPL + G2 REPL. NOTE 2026-08-04: `--list-vars` is still unbuilt, but its data source now exists for real — ADR-0178 made `placement.yaml` a generated, total, gated index derived from `(cljw.internal/__dump-placement)`, so the CLI is a formatting job over `src/runtime/introspect.zig` rather than a yaml-plumbing one                                                                                                                                                                                                           | Quality loop                           | New §9 row                                                                                 | file new D-NNN    |
