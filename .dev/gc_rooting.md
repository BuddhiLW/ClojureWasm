...
`EvalFrame { stack, sp, locals, constants, callee, parent }`, threadlocal head
`eval_frame_head`. Walk drains `stack[0..sp]` → `locals` → `constants` →
`callee` → parent. `callee` (ADR-0184 A) roots the EXECUTING callable —
load-bearing now that Function cells are collectable: the VM drops the callee
below `op_top` at flatten, and tree_walk's per-call frame installs the slot
BEFORE `bindCallFrame` (the binder's rest-pack consHeap can collect).

| #  | Site                                              | Kind                                                     | Roots                                                                                                                                                                                                                                                 |
|----|---------------------------------------------------|----------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| A1 | `eval/backend/vm.zig` (`eval` body)               | VM activation (one per `vm.eval`, chained via `op_call`) | operand `stack[0..sp]`, call-frame `locals`, `constants = chunk.constants` (executing chunk's literal pool, ADR-0095 2a), `callee` per flattened frame (ADR-0184 A)                                                                                   |
...
**UNROOTED-CANDIDATE (tracked D-252; discharged rows marked):**

C1 now roots both eager and bounded apply accumulators and cursors through
EvalFrame, refreshing the off-heap slice after each append. Native sequence
boundary tests force GC inside lazy callbacks and the invoked function.

| #  | Site                                                                               | Held accumulator                                                                                                                                                                                                                                                                                                                                                                                                 | Reentrant call                                                      | Severity                                                                                       |
|----|------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| C1 | **DISCHARGED 2026-09-06** `higher_order.zig` `applyFn` / `applyVariadic`                                                       | `collected: ArrayList(Value)` on `rt.gpa` (off-heap, off-stack)                                                                                                                                                                                                                                                                                                                                                  | `seqFn`/`firstFn`/`nextFn` per spread element                       | HIGH (apply-over-lazy; N-wide off-heap list)                                                   |
...
| D4 | `trackHeap` → `heap_objects` + `gc.registerPersistentMark` → `persistent_marks: ArrayList(*anyopaque)` | process-lifetime gpa objects (`ProtocolDescriptor`, `ProtocolFn`, `TypeDescriptorRef` — `Function` LEFT this set at ADR-0184: fns are ordinary swept GC cells now) OUTSIDE `gc.allocations`; `collect` clears their mark bit each cycle so their trace re-runs (ADR-0095 Class 1)                                                                                             |
| D5 | `trackHeap` callers                                                                                      | `protocol.zig` (×3), `type_descriptor.zig` (×1); the reify anonymous-TD per-evaluation trackHeap is the remaining churn-class occupant (ADR-0184 residual)                                                                                                                                                                                                                   |
| D7 | `mark_sweep.collect` → `type_descriptor.markRegistryRoots` (ADR-0184 B)                                 | EVERY registered descriptor's `method_table[].method_val` + `meta`, across `rt.types` + `rt.native_descriptors` + `rt.class_descriptors` (reached via the `runtime_embedded` @fieldParentPtr bridge); extend-type replaces rows in place so superseded impls stay collectable                                                                                                  |
| D6 | `eval_session.StarState` `gc.pin`/`unpin` (ADR-0170)                                                     | REPL star history `*1`/`*2`/`*3`/`*e` held BETWEEN evals — CLI REPL (one StarState per session) + every nREPL session (`nrepl/session.zig` Registry). Pin on rotate/setE, unpin the displaced value; `release()` on session close / REPL exit. Nothing else references a REPL result between evals — an unpinned `*1` would be swept and the next deref touches freed memory |
...
| E3 | `ThreadGcContext` registry (`registerThread`/`unregisterThread`, fixed `[64]`)         | union of every live worker's `current_frame`+`eval_frame_head`+`gc_self_guard`+`analysis_frame_head`+`tx_slot` (registrants: future.worker, agent.drainer, thread.worker — all via `root_set.workerContext`). Slot reads/writes are atomic acquire/release; **registration is itself a safepoint** (ADR-0175): the newborn parks after publishing if `gc_requested` is set, and on `TooManyThreads` the worker must NOT run its mutator body. Pre-register worker code touches only spawner-pinned objects |
| E4 | safepoint STW (`gc_requested`/`park`/`stopWorld`, target recomputed each wake)         | mechanism (not a source) ensuring workers quiescent during collect; `stopWorld` recomputes the park target from the lock-free `registered_count` each wake + a leaving worker calls `noteWorkerLeft`, so a tiny-action drainer that unregisters before it ever parks cannot hang the collector (D-244 #4). The flag-store/count-read vs count-add/flag-load pairs are seq_cst (ADR-0175 spawn-to-register Dekker handshake — do not relax)                                                                 |
