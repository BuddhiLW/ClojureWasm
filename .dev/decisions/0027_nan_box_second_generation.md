...
**Group D (slots 48..63) — Numeric + Clojure collection internals + wasm tail**
```
...
```
(Layout per amendment 6, the D-248 reorg 2026-06-15 — Clojure persistent-collection
internals up to D4..D8, wasm surfaces at the D12..D15 tail. See the revision history.)

`wasm_funcref` / `wasm_externref` at D14 / D15 are **inline-tagged**, committed by F-004 (decree). zwasm v2 §4.1 encodes `*const FuncEntity` with `align(8)` (F-008) — fits the 45-bit shifted pointer field with margin. The D-036 Phase 16 decision is **only about the marshalling wrapper** (whether `host_func.zig` exposes an additional Pod-shaped wrapper around the inline body for FFI hand-off); the slot encoding itself is decreed inline by F-004. This resolves the internal contradiction Devil's-advocate flagged on the §2 draft (cross-coupling §"wasm_funcref / wasm_externref inline reservation" row).

Group D is now fully named (16/16, no anonymous reserves); D-043's slot-reservation concern is closed for Group D (the remaining D-043 scope is B-band). Per F-002 reservations are memos — naming every slot when a consumer exists is the finished form.

`array` (D3) is Java-array compat (`Object[]` boundary for host interop). `matcher` (D9) is `java.util.regex.Matcher` analogue. `tuple` / `box` (D10 / D11) are reserved for transducer / mutable-box scaffolding (Phase 7).
...
- 2026-06-15 (amendment 6): §2 Group D slot map **reordered** (debt D-248) — the Clojure persistent-collection internals move UP to D4..D8 (`hamt_node`=52 / `tail_node`=53 / `hamt_map_node`=54 / `hash_collision_map_node`=55 / `tval`=56), `matcher`/`tuple`/`box` shift to D9..D11 (57..59), and the four wasm surfaces (`wasm_module`/`wasm_fn`/`wasm_funcref`/`wasm_externref`) move to the D12..D15 **tail** (60..63). Finished-form cleanliness (the user's `layout-gc-decisions-2026-06-05.md` noted wasm was reserved early, pushing Clojure essentials to the end). CHEAP + NON-BREAKING + NOT an F-004 amendment (F-004 fixes 4×16=64 slots + the day-1 type set, not the intra-group ORDER; ADR-0027 §2 owns the slot-map order, loop-amendable). Confirmed safe: runtime dispatch is enum-NAME-based (no hardcoded numeric tag value), the per-Tag GC trace/finaliser tables register by `@intFromEnum(tag)` so they auto-follow the renumber, and `cljw build` serialized bytecode uses a DECOUPLED `ValueTag` (per-Value tag-byte, not raw u64 bits) — no serialized artifact is touched. HeapTag + Value.Tag renumbered in lockstep (the `value.zig` sync test asserts it); `D0 big_int = 48` unchanged; the D15 boundary check (`tag_ops.zig`) updated from `tval` to `wasm_externref`. All values stay in 48..63, so the 2-bit group band is preserved. 1109/1109 units green (-Dwasm). Group D is now fully named 16/16 (D-043's Group-D reserve concern closed; B15 remains). Smell category: none (mechanical finished-form refinement within the F-004 envelope, name-based dispatch verified). Source: `value/heap_tag.zig`, `value/value.zig`, `gc/tag_ops.zig`.
