...
|-----------------|------------------|-------------------------------------------------|
| **boundary**    | `bench/*.sh`, `bench/*.clj` | spawn runtimes, read the clock, write the datum; a cljw-only measurement is a cljw program |
...
./zig-out/bin/cljw -cp bench -m wasm-percall --yaml=bench/wasm-percall-latest.yaml
                                        # one wasm/call crossing in ns, per engine and thread (a cljw program)
...
### Per-crossing cost, one `wasm/call` in nanoseconds

The FFI tables above cannot see a per-call cost: every workload loops inside
the module and crosses the boundary once per process. This is the other axis,
measured by `bench/wasm_percall.clj`, a cljw program (the loop on the cljw
side, only the calls timed, median of trials with the dispersion recorded). The 2026-09-05 finding
that the default engine cost ~16.5 us per call on Linux (zwasm/D-584, the
initial-thread stack query) is the kind of number this table exists to show;
ADR-0195 removed it and ADR-0196 documents what remains.

<!-- BEGIN generated by gen_cross_table.py. Do not edit by hand -->

**Conditions:** Linux amd64, ClojureWasm v1.14.1 (wasm), bench/wasm_percall.clj **1 warmup + 7 runs**, 2026-09-06.

_The loop is on the cljw side and only the calls are timed: one module instantiated once per engine, then trials of N calls to a trivial `add` export. This is the axis the FFI table above cannot see (its workloads loop inside the module). `fn-call` is a plain Clojure function call in the same process, the platform constant a crossing is measured against. `-worker` rows run on a `future` thread. Engines have opposite preferences: compute-bound bodies favour the JIT (~15x, see `bench/wasm_jit_vs_interp.sh`), crossing-heavy calls favour the interpreter._

_Measurement conditions: median run-to-run dispersion **24%**, so ratios under **1.68x** are not distinguishable from noise._

#### One `wasm/call` crossing, median of trials (ns, lower is better)

| Benchmark | ClojureWasm |
|---|---|
| auto-main | 1222 |
| auto-worker | 999 |
| jit-main | 1193 |
