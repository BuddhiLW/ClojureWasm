...
//! implementation.
//!
//! Layer-0 module: imports only `runtime/` siblings (value, keyword,
//! the heap collection wrappers, numeric/regex/uuid/time value types,
//! env, dispatch). No analyzer / backend / `lang/` knowledge — the
//! printer is data-driven off `Value.tag()`; renders that need eval
//! (lazy-seq forcing, protocol dispatch, `print-method`) reach it via
//! injected accessors and threadlocal rt/env contexts.
//!
//! ### Surface
//!
//! - `printResult(rt, env, w, v)` — the user-facing readable entry
//!   (REPL / `-e` result, nREPL, `prn`): snapshots the `*print-…*`
//!   control vars, deep-realizes nested lazy seqs, then renders.
//! - `printConsult(rt, env, w, v)` — `printResult` plus a
...
/// this so lazy results render as `(…)`, not `#<lazy_seq>`. The realize
/// is DEEP — nested lazy seqs in the seq family (lazy_seq / list /
/// vector) and in map values / set elements (all four map/set tags) are
/// forced too, so `(partition-by …)` / `(split-at …)` /
/// `(into [] (partition-all …))` and a lazy children seq assoc'd into a
/// map (potpuri's build-tree; corpus `lazy_nested_print`) render by
/// content, not `(#<lazy_seq> …)`.
pub fn printResult(rt: *Runtime, env: *env_mod.Env, w: *Writer, v: Value) anyerror!void {
    // Snapshot *print-length*/*print-level* once per top-level value
...
/// Object-method fallback (Layer 1) — one impl, both surfaces. Top-level string
...
pub fn writeStrValue(rt: *Runtime, env: *env_mod.Env, w: *Writer, v: Value) anyerror!void {
    switch (v.tag()) {
        ...
        // form `pr`/`prn` use. Normal floats render identically under both,
        // so delegate to `printFloat`.
        .float => {
            const f = v.asFloat();
            if (std.math.isNan(f)) {
                try w.writeAll("NaN");
        ...
        // literal stays a pr/prn-only form — `str` never emits it.
        .ex_info => {
        ...
        // emit plain digits. Ratio's `1/2` form is already suffix-free.
        .big_int => try w.print("{f}", .{big_int_mod.asManaged(v)}),
        .big_decimal => try writeBigDecimalDigits(w, v),
        // A deftype/reify `Object (toString [this] …)` impl supplies the
...
fn listFromItems(rt: *Runtime, items: []const Value) !Value {
    // Empty → the interned empty list, so a realized empty seq
...
fn realizeSeqWalk(rt: *Runtime, env: *env_mod.Env, v: Value, depth: i64) anyerror!Value {
    ...
    // collect sweeps a partly-realized nested seq -> garbage cons (the
    ...
    while (true) {
        ...
        // *print-length*: bound realization to limit+1 items so an
...
/// infinite recursion (a segfault, historically).
...
/// marker protocol — the discriminator for "prints as a seq" (see the
/// deepRealize arm). Shares `declaresProtocol` with `sequential?` (one SSOT).
...
/// formatter; the analyzer's Form printer (`eval/form.zig`) delegates
/// here. Acceptable divergence: the smallest subnormal
...
/// `print`-form of a char: the bare UTF-8 character, no `\` literal
/// prefix. Mirrors `writeArgsSpaced`'s top-level raw-char path
/// (`lang/primitive/core.zig`) for chars nested inside a collection
/// printed under `print`/`println`.
...
/// Render a char in readable (`pr`) form, clj-faithful: named forms for
/// the 6 standard whitespace chars, else `\` + the literal char.
pub fn printCharReadable(w: *Writer, cp: u21) anyerror!void {
    switch (cp) {
        ...
        12 => try w.writeAll("\\formfeed"),
            ...
            // alike. clj emits NO \uXXXX escape (oracle-verified:
            // (char 7) -> `\`+BEL, (char 233) -> `\`+the UTF-8 char).
            try w.writeByte('\\');
...
/// Clojure's `*print-readably*`, as a thread-local since the pure
...
/// it. Set/restored by `writeArgsSpaced` alongside
...
/// A callable value's qualified identity for printing (ADR-0121). Both
...
/// Reads a `.fn_val`'s `{ns, name}`. `.fn_val`'s `Function` struct is
...
/// Borrowed reverse-index `builtin_fn ptr → {ns, name}` (owned by `Env`).
...
/// Clojure's `*print-namespace-maps*`. When true and every key of a
...
/// The Var IDENTITY is fixed after intern, so these are
...
/// `*print-namespace-maps*` cached Var. Unlike the limits it
...
/// `*print-readably*` cached Var. Unlike the others it is
...
/// `*print-dup*` cached Var. Only consulted to FAIL LOUD:
/// a truthy binding asks for clj's JVM `#=(class/create …)` ctor forms, which
/// cljw (no JVM classes) cannot represent — the pr surface raises instead of
...
/// lands here terminates. Bypassing the consult on the same value is the
/// recursion invariant that keeps user methods terminating.
...
/// (ADR-0088: deref once at the surface, not per element). null =
...
fn snapshotPrintLimits() void {
    ...
    print_depth = 0;
    // *print-namespace-maps*: truthy → compact `#:ns{…}`. Root is
    ...
    // *print-readably*: override the surface-set flag ONLY when the
...
/// The shared namespace of an array_map's keys, or null when the map
...
fn printValueNative(ports: ?Ports, w: *Writer, v: Value) anyerror!void {
    ...
    // *print-meta*: emit a `^{meta} ` prefix for any value
    ...
    switch (vtag) {
        ...
        // A builtin prints its qualified name `#<rt/+>` via the
        ...
        .uuid => {
            ...
            // special-cases `.uuid` before reaching here.
            const canon = uuid_mod.canonicalOf(v);
            try w.writeAll("#uuid \"");
            try w.writeAll(&canon);
            try w.writeByte('"');
        ...
        .tagged_literal => {
            // `#<tag> <form>` — EDN round-trips. tag is a symbol,
        ...
        // hash cljw cannot mirror. EXCEPTION: a host-enum constant
        // (RoundingMode/ChronoUnit/DayOfWeek/Month) prints its bare `toString`
        // (the enum/display name).
        .host_instance => {
            const inst = @import("host_instance.zig").asHostInstance(v);
        ...
        // AD-025: callable values print `#<ns/name>`, not the leaked
...
fn printBigInt(w: *Writer, v: Value) anyerror!void {
    ...
    // marks a genuine BigInt. A heap-boxed Long prints
...
fn printRatio(w: *Writer, v: Value) anyerror!void {
    // Render numerator `/` denominator without a trailing `N` (small =
...
fn printBigDecimal(w: *Writer, v: Value) anyerror!void {
    ...
    // drop it (JVM `BigDecimal.toString` has no suffix).
    try w.writeByte('M');
}
...
/// `map.forEachEntry` accumulator for a record's extmap: print each
...
fn printTypedInstance(ports: ?Ports, w: *Writer, v: Value) anyerror!void {
    ...
    // A record prints map-style `#ns.Name{:k v, …}` from its declared field
    // layout + any extmap entries,
    // ns-QUALIFIED like clj (the reader constructs the record back from
    // this form). The ns-less form appears only for a descriptor with no
    // defining ns (bare unit-test registration).
...
/// emits `#object[Namespace "user"]` (AD-010).
...
fn printHamtEntries(ports: ?Ports, w: *Writer, node: *const map_collection.HamtMapNode, first: *bool, count: *i64, stop: *bool, comptime kv: bool) anyerror!void {
    ...
    while (i < data_count) : (i += 1) {
        if (stop.*) return;
        // *print-length*: emit `...` (with separator unless first)
...
test "unhandled heap tag falls back to #<tag>" {
    ...
    // never the pointee. (`.fn_val` was the prior stand-in; it has a real
    // `#<ns/name>` arm now, so it no longer exercises the fallback.)
